...
export default function BidderDashboard({ room, timerStatus, actions, currentUser, recentBids }) {
  ...
  const [sideTab, setSideTab] = useState('leaderboard'); // 'leaderboard' | 'inventory' | 'activity'
              ...
              <div className="mb-8">
                <span className="px-4 py-1.5 rounded-full bg-gray-800 text-gray-400 border border-gray-700 text-sm font-bold uppercase tracking-wider">
                  {room.state.status === 'waiting' ? 'Waiting for Host...' :
                   ...
                   room.state.status === 'roundEnded' ? 'Round Ended — Next item coming...' : 'Waiting...'}
                </span>
              </div>
            )}
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="glass-card p-2 flex gap-2">
          {[
            ...
            { key: 'inventory', label: `Inventory (${myInventory.length})`, icon: <Package size={16} /> },
            { key: 'activity', label: 'Activity', icon: <Activity size={16} /> }
          ].map(tab => (
            <button
              key={tab.key}
              onClick={() => setSideTab(tab.key)}
              className={`flex-1 flex items-center justify-center gap-1.5 py-2 px-2 rounded-xl text-xs font-bold transition-colors ${sideTab === tab.key ? 'bg-primary/30 text-white border border-primary/50' : 'text-gray-400 hover:text-white'}`}
            >
              {tab.icon} {tab.label}
            </button>
          ))}
        ...
        {sideTab === 'inventory' && (
          <div className="glass-card p-6 flex-1">
            <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
              <Package size={20} className="text-purple-400" /> My Inventory
            </h2>
            {myInventory.length === 0 ? (
              <p className="text-gray-500 text-center py-8">No items won yet. Start bidding!</p>
            ) : (
              <div className="space-y-3">
                {myInventory.map((item, i) => (
                  <div key={i} className="flex flex-col p-3 bg-purple-900/20 rounded-xl border border-purple-500/30">
